...
- `done`: Append completion date and set `Current step` to `none`. **This MUST be done immediately after presenting the `walkthrough.md`.**

## 5. [PROHIBITED ACTIONS]
- **[MUST [S-BM-1]]**: NEVER leave the `Status` or `Current step` descriptors outdated in `docs/backlog.md` after a phase transition. You are OBLIGATED to sync the backlog physically.
